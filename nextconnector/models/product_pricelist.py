...
class ProductPriceList(models.Model):
    ...
    def post_product_pricelist(self, params):

        data_obj = self.env["nextconnector.template_query"].search([('code','=',"data_product_pricelist"),('state','=',True) ], limit=1)
        if not data_obj: 
            return True
        ...
        if response["status_code"] != "200":
            ...
            if json_resp["code"] != "0":
                ...
                for list_data in json_resp["list_data"] :
                    try:
                        data_obj = self.env["nextconnector.template_query"].search([('code','=',"normalize_data_product_pricelist"),('state','=',True) ], limit=1)
