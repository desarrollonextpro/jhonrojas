...
class Process(models.TransientModel):
    ...
    def import_items(self):
        ...
        data_obj = self.env["nextconnector.template_query"].search([('code','=',"list_products"),('state','=',True) ], limit=1)
        ...
        response = _rq(self,"/nextconnector/api/records/items",results_vars["json_data"],"get")
            ...
            if json_resp["code"] != "0":
                ...
                for list_data in json_resp["list_data"] :
                    ...
                    for row in list_data["data"] :
                        _product.ProductTemplate.post_product(self, row)

                return _popup.success(self, "Importación culminada.")
                
    def import_customers(self):

        data_obj = self.env["nextconnector.template_query"].search([('code','=',"list_customers"),('state','=',True) ], limit=1)
        if not data_obj: 
            return True
        ...
        response = _rq(self,"/nextconnector/api/records/customers",results_vars["json_data"],"get")
        ...
        if response["status_code"] != "200":
            ...
            if json_resp["code"] != "0":
                ...
                for list_data in json_resp["list_data"] :
                    ...
                    for row in list_data["data"] :
                        _customer.ResPartner.post_customer(self, row)

                return _popup.success(self, "Importación culminada.")

    def import_stock_inventory(self):

        data_obj = self.env["nextconnector.template_query"].search([('code','=',"stock_products"),('state','=',True) ], limit=1)
        if not data_obj: 
            return True
        ...
        if response["status_code"] != "200":
            ...
            if json_resp["code"] != "0":
                ...
                for list_data in json_resp["list_data"] :        
                    try:
                        data_obj = self.env["nextconnector.template_query"].search([('code','=',"normalize_stock_products"),('state','=',True) ], limit=1)
                        if data_obj.state != True: 
                                return True
    ...
    def import_product_pricelist(self):
        ...
        data_obj = self.env["nextconnector.template_query"].search([('code','=',"list_product_pricelist"),('state','=',True) ], limit=1)
        if data_obj.state != True: 
                return True
        ...
        if response["status_code"] != "200":
            ...
            if json_resp["code"] != "0":
                ...
                for list_data in json_resp["list_data"] :
                    for row in list_data["data"] :
                        _pricelist.ProductPriceList.post_product_pricelist(self, row)

                return _popup.success(self, "Importación culminada.")

    def get_data_query(self, query_string=""):
    ...
    def post_generic_request(self, record, url_route, template_data_code):

        if not url_route or not template_data_code: 
                return True

        data_obj = self.env["nextconnector.template_data"].search([('code','=',template_data_code)], limit=1)
        if data_obj.state != True: 
            return 
        ...
        response = _rq(self,url_route,results_vars["json_data"],"post")
    ...
    def exec_template_code(self, record, template_data_code):
    ...
    def generic_request(self, record, url_route, method, template_type, template_code, id_process_queue):

        if not url_route or not template_code: 
                return True

        data_obj = self.env["nextconnector.template_" + template_type].search([('code','=',template_code)], limit=1)
        if data_obj.state != True: 
        ...
        response = _rq(self,url_route,results_vars["json_data"], method)
