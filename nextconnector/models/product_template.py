...
class ProductTemplate(models.Model):
    ...
    def post_product(self, params):

        data_obj = self.env["nextconnector.template_query"].search([('code','=',"data_product"),('state','=',True) ], limit=1)
        if not data_obj: 
            return True
            ...
            if json_resp["code"] != "0":
                ...
                for list_data in json_resp["list_data"] :
                    for row in list_data["data"] :
                        try:
                            data_obj = self.env["nextconnector.template_query"].search([('code','=',"normalize_data_product"),('state','=',True) ], limit=1)
